...
- **할당량 문제 발생 시 오류 메시지**: 할당량 문제가 발생했을 때 "Quota exceeded! Change model!" 이라는 오류 메시지가 나타납니다.

### 변경됨

- Gemini Pro 모델은 할당량 문제가 자주 발생되어, 지원 모델에서 제외합니다.
- API에서 더 이상 지원되지 않는 오래된 모델을 제외합니다.

## [0.7.0] - 2025-08-27

### 추가됨

- **WebView 내 Gemini 모델 선택 기능**: 사용자가 웹뷰 패널에서 직접 오류 분석에 사용할 Gemini 모델을 선택할 수 있는 드롭다운 메뉴를 추가했습니다. 이를 통해 사용자는 필요에 따라 속도, 비용, 성능 등 원하는 모델을 유연하게 선택할 수 있습니다.
- **신규 모델 지원**: 다음 Gemini 모델 목록을 지원합니다:
  - `gemini-2.0-flash`
