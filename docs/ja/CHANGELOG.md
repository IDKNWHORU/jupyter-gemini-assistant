...
- **割り当て制限時のエラーメッセージ**: 割り当て（Quota）の問題が発生した際、「Quota exceeded! Change model!」というエラーメッセージが表示されるようになりました。

### 変更

- 割り当て制限の問題が頻発するため、Gemini Proモデルをサポート対象から除外しました。
- APIでサポートされなくなった古いモデルを削除しました。
